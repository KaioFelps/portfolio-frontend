...
export abstract class AdminPagesRootActionsHandlers {
	public static async load(this: ServerLoadEvent) {
		const [logs, statistics] = await Promise.all([
			AdminPagesRootActionsHandlers.fetchLogs(this),
			AdminPagesRootActionsHandlers.fetchStatistics(this),
		]);
	...
	private static async fetchLogs({
		fetch,
		locals,
	...
	private static async fetchStatistics({
		fetch,
		locals,
